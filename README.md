...
const client = createClient({
  ...
  beforeCall: function(call){
    let { metadata, request } = call;
    let authService = require('./path/to/auth.service');
...
|**beforeCall**|(call) => void \| Promise | Optional | event hook to execute a function before a rpc is called. This is setup on a per-sevice basis|
|**afterCall**|(call) => void \| Promise | Optional | event hook to execute a function after a rpc is called. This is setup on a per-sevice basis|
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2MTc5OTczMzQsODY2ODk2MTEwXX0=
-->